...
export default function SearchProduct() {
  ...
  const nigeriaState = useSearchParams().get('state');
  ...
  const fetchProducts = useCallback(async (page, search) => {
    try {
      ...
      if (search || nigeriaState) {
        query = query.or(`title.ilike.%${search}%.user_profile.stat.eq.${nigeriaState}`);
      } else {
        if (search) {
          query = query.ilike('title', `%${search}%`);
        }
        if (nigeriaState) {
          query = query.ilike('user_profile.stat', nigeriaState);
        }
      }
